...
//                .baseUrl("http://" + System.getProperty("host") + ":" + System.getProperty("port") + "/")
    ...
    public void updateTag(String tagId, JsonNode tag, ServiceCallback callback) {
        Call<JsonNode> call = apiService.updateTag(tagId, tag);
        call.enqueue(new JsonCallback(callback));
    }

    public void deleteTagById(String tagId, ServiceCallback callback) {
        Call<JsonNode> call = apiService.deleteTagById(tagId);
        call.enqueue(new JsonCallback(callback));
    }
